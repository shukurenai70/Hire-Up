...
export default function StudentDashboardPage() {
    ...
    useEffect(() => {
        const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
            if (user && user.email) {
                // Get student data
                const studentLookupRef = doc(db, 'Students', user.email);
                const unsubscribeStudent = onSnapshot(studentLookupRef, (lookupSnap) => {
                    if (lookupSnap.exists()) {
                        const studentData = lookupSnap.data() as Student;
                ...
                const notifQuery = query(
                    ...
                    limit(5) // Fetch more initially to filter on client
                );
                const unsubscribeNotifs = onSnapshot(notifQuery, (snapshot) => {
                    const allPublished = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Notification));
                    // Client-side filter
                    const filteredNotifs = allPublished
                        .filter(n => n.target === "All Students" || n.target === student?.course)
                        .slice(0, 3);
